...
module TingYun::LanguageSupport
  extend self

  # need to use syck rather than psych when possible
  def needs_syck?
    !TingYun::LanguageSupport.using_engine?('jruby') &&
        TingYun::LanguageSupport.using_version?('1.9.2')
  end
  ...
  def broken_gc?
    TingYun::LanguageSupport.using_version?('1.8.7') &&
        RUBY_PATCHLEVEL < 348 &&
        !TingYun::LanguageSupport.using_engine?('jruby') &&
        !TingYun::LanguageSupport.using_engine?('rbx')
  end
